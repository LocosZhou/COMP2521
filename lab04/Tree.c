...
static Link insertSplay (Link t, Item it)
{
	...
	if (diff == 0) {
		...
		if (t->left == NULL) {
			t->left = newNode (it);
		} else if (thisTree->ncompares++ && less (v, key (t->left->value))) {
		...
		if (t->right == NULL) {
			t->right = newNode (it);
		} else if (thisTree->ncompares++ &&less (key (t->right->value), v)) {
...
static Link searchSplay (Link t, Key k, int *found)
{
	...
	if (t == NULL) {
		...
		*found = 0;
		res = NULL;
	} else if (thisTree->ncompares++ && eq (key (t->value), k)) {
		*found = 1; // item found, store true
		res = t;
	} else if (thisTree->ncompares++ && less (k, key (t->value))) {
		if (t->left == NULL) {
			*found = 0; // item not found
			// res = rotateRight(t);
			res = t;
		} else if (thisTree->ncompares++ && eq (key (t->left->value), k)) {
			*found = 1;
			res = rotateR (t);
			thisTree->nrotates++;
		} else {
			if (thisTree->ncompares++ && less (k, key (t->left->value))) {
				// left-left
		...
		if (t->right == NULL) {
			*found = 0; // item not found
			// res = rotateLeft(t);
			res = t;
		} else if (thisTree->ncompares++ && eq (key (t->right->value), k)) {
			*found = 1;
			res = rotateL (t);
			thisTree->nrotates++;
		} else {
			if (thisTree->ncompares++ && less (key (t->right->value), k)) {
				/* right-right */
				t->right->right =
